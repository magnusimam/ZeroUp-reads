...
|---|---|---|
| `statsService.js`/`testimonialsService.js` should swap to real API calls | API-First Design | `booksService.js`, `authService.js` (including user/role management), `userService.js` (reading progress), `bookmarksService.js`, and `publishingService.js` now all call the real `backend/` API behind `realXApi` feature flags (Stages 4–12). `GET /analytics` now exists too (Stage 13) but `statsService.js` isn't wired to it yet — same "backend built, frontend swap is a separate stage" gap; `testimonialsService.js` still has no backend endpoint at all. |
...
| Modules for Languages, Translation, Audio, Search, Recommendations, Users, CMS, AI, Notifications | Domain-Driven Design | Add as those features get built — not worth pre-creating empty folders. |
